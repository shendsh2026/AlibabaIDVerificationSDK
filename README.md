# CUniqAlibabaIDVerificationSDK

Binary CocoaPods package for the CUnique Alibaba ID Verification UTS plugin.
...
`uni_modules/CUniq-Alibaba-ID-Verification/utssdk/app-ios`
...
- `CUniqAlibabaIDVerificationSDK.podspec`: CocoaPods binary podspec
- `strip-report.txt`: record of the safe `strip -S` pass applied to the copied binaries

## Publish

Replace the placeholder Git URL in `CUniqAlibabaIDVerificationSDK.podspec`, then push the repository and tag:
...
```bash
mkdir -p /tmp/cuniq-pod-test
cd /tmp/cuniq-pod-test
pod init
```
...
target 'TestApp' do
  pod 'CUniqAlibabaIDVerificationSDK',
...
`uni_modules/CUniq-Alibaba-ID-Verification/utssdk/app-ios/config.json`
